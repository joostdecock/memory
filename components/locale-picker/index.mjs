...
// Components
import { I18nIcon, DownIcon } from 'components/icons.mjs'
import en from 'site/public/locales/en/locales.json'
import cn from 'site/public/locales/cn/locales.json'
                ...
                <div className="relative grid gap-2 bg-base-100 p-4 grid-cols-1">
                  {router.locales.map((locale) => (
                    <Link
                      ...
                      key={locale}
                      locale={locale}
                      className="btn btn-neutral btn-outline flex flex-row items-center justify-between"
                    >
                      <span>{en[locale]}</span>
                      <span>{cn[locale]}</span>
                    </Link>
